    ...
    if ($operacao == "new") {
        if ((isset($_GET["diretorio"])) &&
            ...
            if (($SQLUsuario->rowCount()) > 0) {
                echo myalertuser('warning', 'O usuário ' . $_GET['login'] . ' já existe!', $diretorio);
                exit;
            }
            ...
            if ($tipouser == 'erro') {
                echo myalertuser('warning', 'Selecione o tipo de usuário', $diretorio);
                exit;
            }
            if ($tipouser == 'revenda') {
                $subrevenda = 'sim';
                if ($eumesmo['subrevenda'] == 'sim') {
                    echo myalertuser('warning', 'No momento você não pode adicionar revenda!', $diretorio);
                    exit;
                }
            } else {
                $subrevenda = 'nao';
            ...
            $type_access = $_GET["acesso"];
            $SQLNew = "INSERT INTO usuario (ativo, id_mestre, nome, login, tipo, subrevenda, celular, senha, data_cadastro, token_user, permitir_demo)
                                            VALUES ('1', '" . $owner . "', '" . $nome . "', '" . $login . "','" . $tipouser . "' , '" . $subrevenda . "',  '" . $celular . "', '" . $senha . "', '" . $date . "' , '" . $token . "' , '" . $type_access . "') ";
            ...
            /*
        $mensagem = "Seja Bem vindo! Nos Dados de acesso: IP->".$endereco_web." Login-> ".$login." Senha->".$senha;
        ...
        $SQLSMS->execute();
            */
            ...
            echo info_alert_user($administrador['site'] .'', ''. $_GET['login'] .'', ''. $_GET['senha'] .'', '../../home.php?page=usuario/perfil&id_usuario='.$id['last_id'] .' ');
            exit;
        } else {
        ...
        if ((isset($_GET["id_usuario"])) &&
            ...
            $SQLUpdate->execute();
            echo myalertuser('success', 'Dados alterados!', $diretorio);
            exit;
        } else {
            echo myalertuser('warning', 'Preencha todos os campos!', $diretorio);
            exit;
        ...
        if ((isset($_GET["id_usuario"])) &&
            ...
            $SQLSMS = $conn->prepare($SQLSMS);
            $SQLSMS->execute();
            echo myalertuser('success', 'Senha reenviada !', $diretorio);
            exit;
        } else {
            echo myalertuser('warning', 'Preencha todos os campos!', $diretorio);
            exit;
            ...
            if ($usuario['tipo'] == "revenda") {
                ...
                if (($SQLUsuarioSub->rowCount()) > 0) {
                    while ($row = $SQLUsuarioSub->fetch()) {
                        ...
                        $SQLExcluiSRev = $conn->prepare($SQLExcluiSRev);
                        $SQLExcluiSRev->execute();

                        $SQLExcluiAcessor = "delete  from acesso_servidor WHERE  id_usuario = '" . $row['id_usuario'] . "'";
                        $SQLExcluiAcessor = $conn->prepare($SQLExcluiAcessor);
        ...
        if ((isset($_GET["id_usuario"])) &&
            ...
            $SQLUser = $conn->prepare($SQLUser);
            $SQLUser->execute();
            echo myalertuser('success', 'Suspendido com sucesso !', $diretorio);
            exit;
        } else {
        ...
        if ((isset($_GET["id_usuario"])) &&
            ...
            if (isset($_GET['renovar'])) {
                ...
                $add = date('Y-m-d H:i:s', strtotime('+30 days'));
                $SQLSucesso = "update acesso_servidor set validade='" . $add . "' where id_acesso_servidor='" . $_GET['renovar'] . "'";
                $SQLSucesso = $conn->prepare($SQLSucesso);
                $SQLSucesso->execute();
                echo myalertuser('success', 'Renovado com sucesso !', $diretorio);
                exit;
            }else{
                echo myalertuser('success', 'Reativado co sucesso !', $diretorio);
                exit;
            }
        } else {
            echo myalertuser('warning', 'Preencha todos os campos!', $diretorio);
