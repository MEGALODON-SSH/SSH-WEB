...
if(($SQLSSH->rowCount()) > 0){
    while($row = $SQLSSH->fetch()){
        ...
        $ssh->exec("usermod -L ".$row['login']."");
        $ssh->output();
        $ssh->exec('[[ -f "/opt/sshplus/sshplus" ]] && /opt/sshplus/plugin-sync --kill_user '.$row['login'].' || ./KillUser.sh '.$row['login'].'');
        $ssh->output();
