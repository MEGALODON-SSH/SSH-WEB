...
if (isset($_POST['email'])) {
    ...
    $buscanosm = "SELECT * FROM smtp_usuarios WHERE usuario_id='" . $_SESSION['usuarioID'] . "'";
    ...
    $empresanome = $smtp['empresa'];
    $empresaemail = $smtp['email'];
    // Compo E-mail
	...
	";
        ...
        if ($tip == 'sshconta') {
            ...
            $portas_corpo = "<b>Portas Proxy</b>: 80,8080,8799,3128 <br />";
    ...
    $de = explode("@", $email);
    //enviar
    $assunto = "[" . strtoupper($assunto) . "]" . "[" . strtoupper($empresanome) . "]";
    $corpo = $arquivo;
    $destino = $email;
    $destinatario = strtoupper($de[0]);
    $sucesso = 'Email enviado, o seu cliente receberá na caixa de entrada.';
    ...
    $mail->FromName = strtoupper($empresanome); // Seu nome
